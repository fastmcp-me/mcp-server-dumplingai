...
server.tool("get-youtube-transcript", {
...
}, async ({ videoUrl, includeTimestamps, timestampsToCombine, preferredLanguage, }) => {
    ...
    try {
        const response = await fetch(`${NWS_API_BASE}/api/v1/get-youtube-transcript`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to fetch YouTube transcript: ${response.status} ${errorText}`);
        }
...
server.tool("search", {
    ...
    scrapeOptions: z
        ...
        .optional(),
}, async ({ query, country, location, language, dateRange, page, scrapeResults, numResultsToScrape, scrapeOptions, }) => {
    // Get API key from environment variable
    const apiKey = process.env.DUMPLING_API_KEY;
    if (!apiKey) {
        throw new Error("DUMPLING_API_KEY environment variable not set");
    ...
    try {
        ...
        const formattedResults = {
            ...
            organicResults: data.organic.map((result) => ({
                title: result.title,
                link: result.link,
...
server.tool("search-maps", {
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Failed to perform maps search: ${response.status} ${errorText}`);
        }
        const data = await response.json();
        return {
            content: [
...
server.tool("search-places", {
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Failed to perform places search: ${response.status} ${errorText}`);
        }
        const data = await response.json();
        return {
            content: [
...
server.tool("search-news", {
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Failed to perform news search: ${response.status} ${errorText}`);
        }
        const data = await response.json();
        return {
            content: [
...
server.tool("get-google-reviews", {
    placeId: z.string().optional(),
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(`Failed to get Google reviews: ${response.status} ${errorText}`);
        }
        const data = await response.json();
        return {
            content: [
